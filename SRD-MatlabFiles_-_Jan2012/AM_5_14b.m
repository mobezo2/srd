...
gamma_list = [ 0, 1, 2 ,4,6,10];
gamma_length = size(gamma_list);
pl = gamma_length(2);                           % modulate with carrier
...
for idx = 1:pl
  ...
  % used to plot figure
  figure(idx)
  title_cust = sprintf("(a) message signal 1' %f", gam);
  ...
  axis([0,0.1, -2.5,2.5])  
  title_cust = sprintf("(b) message signal 1 after modulation' %f", gam);
  ylabel('amplitude'); title(title_cust);
  
  subplot(5,1,3), plot(t,x1)
  axis([0,0.1, -1,3])
  ylabel('amplitude');  
  title_cust = sprintf("(c) demodulated signal 1' %f", gam);
  title(title_cust);
    
  subplot(5,1,4), plot(t,s1)  
  axis([0,1, -1,10])  
  title_cust = sprintf("(d) recovered message 1 is a LPF applied to (c)' %f", gam);
  ylabel('amplitude'); title(title_cust);
  
  delta = abs(m1.-s1);
  subplot(5,1,5), plot(t,delta);
  axis([0,1, -1,3])  
  title_cust = sprintf("RMS Between Sent and Received' %f", gam);
  ylabel('amplitude'); title(title_cust);
