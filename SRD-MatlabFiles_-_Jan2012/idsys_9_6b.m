...
%9.6a --- add random PAM symbols
ranpam = [-1 -2 -1 -2 -1 -1 -1 -2 -3 -3 -2 -1 -2 -1 -2 -1 -2 -3 ];
%9.6b --- marker sequence
%header = [-1 -2 -3 3 2 1 2 1 2 3 1 2 3 1 ];
header = [1 2 3 1 2 3 1 2 3 1 2 ];
...
corres =xcorr(header, z);                 % do cross correlation
[temp,ind]=max(corres);                        % location of largest correlation
fl = ind;
corres;
headstart = length(z)-ind+1;
msg_start = headstart + length(header);
z = z(msg_start:1:length(z));
stem(corres)
